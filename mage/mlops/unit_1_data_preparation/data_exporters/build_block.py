...
from mlops.utils_n.data_preparation.encoders import vectorize_features
...
def test_training_set(
    ...
    *args,
    ...
    ),f'Training set for training should have 54378 examples, but has {X_train.shape[0]}'
    assert(
        X_train.shape[1]==5094
    ),f'Training set for training should have 5094 features, but has {X_train.shape[1]}'
    assert(
        len(y_train.index) == X_train.shape[0]
    ),f'Training ser for training model should have {X_train.shape[0]} examples, but has {len(y_train.index)}'
...
def test_validation_set(
    ...
    *args,
    ...
    ),f'Training ser for training model should have {X_val.shape[0]} examples, but has {len(y_val.index)}'
